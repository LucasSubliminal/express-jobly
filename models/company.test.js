...
describe("update", function () {
  ...
  test("works for admin", async function () {
    let company = await Company.update("c1", updateData, { isAdmin: true });
    expect(company).toEqual({
      handle: "c1",
      ...updateData,
    });
  ...
  test("unauth for non-admin", async function () {
    try {
      await Company.update("c1", updateData, { isAdmin: false });
      fail();
    } catch (err) {
  ...
  test("not found if no such company", async function () {
    try {
      await Company.update("nope", updateData, { isAdmin: true });
      fail();
    } catch (err) {
      expect(err instanceof NotFoundError).toBeTruthy();
    }
  ...
  test("bad request with no data", async function () {
    try {
      await Company.update("c1", {}, { isAdmin: true });
      fail();
    } catch (err) {
      expect(err instanceof BadRequestError).toBeTruthy();
    }
...
describe("remove", function () {
  test("works for admin", async function () {
    await Company.remove("c1", { isAdmin: true });
  ...
  test("unauth for non-admin", async function () {
    try {
      await Company.remove("c1", { isAdmin: false });
      fail();
    } catch (err) {
      expect(err instanceof UnauthorizedError).toBeTruthy();
    }
  ...
  test("not found if no such company", async function () {
    try {
      await Company.remove("nope", { isAdmin: true });
      fail();
    } catch (err) {
      expect(err instanceof NotFoundError).toBeTruthy();
    }
